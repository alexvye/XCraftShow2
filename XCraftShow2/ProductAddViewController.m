...
- (void)viewDidLoad
{
    ...
    if(selectedProduct == nil) {
        ...
        NSLog(@"[%@] [%@]",product.defaultCost,product.unitCost);
    }
}
...
-(IBAction) saveProduct: (UIButton*) aButton {
    ...
    if(self.selectedProduct == nil && [self doesProductExist:self.name.text]) {
        ...
        product.unitCost = [CURRENCY_FORMATTER numberFromString:self.unitCost.text];
        product.productDescription = self.productDescription.text;
        product.defaultCost = [CURRENCY_FORMATTER numberFromString:self.defaultCost.text];
        product.image = 0;
